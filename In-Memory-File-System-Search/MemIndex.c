...
LinkedList* MemIndex_Search(MemIndex* index, char* query[], int query_len) {
  ...
  for (i = 1; i < query_len; i++) {
    ...
    for (j = 0; j < num_docs; j++) {
      SearchResult* store;
      LLIterator_Get(ll_it, (LLPayload_t *) &store);
      bool InTable = HashTable_Find(wp->postings, store->doc_id, &kv);
      if (!InTable) {
        LLIterator_Remove(ll_it, (LLPayloadFreeFnPtr)free);
      } else {
        store->rank = store->rank + LinkedList_NumElements((LinkedList *) kv.value);
