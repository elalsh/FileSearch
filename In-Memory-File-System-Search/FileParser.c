...
char* ReadFileToString(const char* file_name, int* size) {
  ...
  if (result == -1) {
    return NULL;  // stat returned error, so we return NULL
  }
  ...
  if (!S_ISREG(file_stat.st_mode)) {
    return NULL;  // Must be a regular file
  }
  ...
  while (left_to_read > 0) {
    ...
    if (num_read == -1) {
      if (errno != EINTR && errno != EAGAIN) {
        close(fd);
        free(buf);
        return NULL;  // Can't continue, return null
       }
       continue;
    } else if (num_read == 0) {
      break;
...
static void InsertContent(HashTable* tab, char* content) {
  ...
  while (1) {
    ...
    if (!isalpha(*cur_ptr)) {  // invalid characer

      // check we have valid word and add to word position
      if (word_start != cur_ptr && isalpha(*word_start) != 0) {
        *cur_ptr = '\0';
...
static void AddWordPosition(HashTable* tab, char* word,
  ...
  if (HashTable_Find(tab, hash_key, &kv)) {
    ...
    HTKeyValue_t toBeInserted;
    int totalLen = strlen(word) + 1;

    // allocate and set new word position
    wp = (WordPositions*) malloc(sizeof(WordPositions));
    Verify333(wp != NULL);
    wp->word = (char*) malloc(sizeof(char) * totalLen);
    Verify333(wp->word != NULL);
    wp->word = strncpy(wp->word, word, totalLen);
    wp->positions = LinkedList_Allocate();
    Verify333(wp->positions != NULL);

    // append the position to the linked list of positions
    LinkedList_Append(wp->positions, (LLPayload_t) (int64_t) pos);

    // set the key and velue of the kv pair to be inserted and inser into table
    toBeInserted.key = hash_key;
    toBeInserted.value = (HTValue_t) wp;

    HashTable_Insert(tab, toBeInserted, &kv);
  }
}
