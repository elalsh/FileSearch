...
// A function used to free the values of the id_to_name and
// name_to_id
// static void FreeValues(LLPayLoad_t payload);
...
DocID_t DocTable_Add(DocTable* table, char* doc_name) {
  ...
  doc_copy = strncpy(doc_copy, doc_name, strlen(doc_name) + 1);
