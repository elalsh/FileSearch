...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  // STEP 1
  char* root_abs_path = realpath(root_dir.c_str(), nullptr);
  if (root_abs_path == nullptr) {
    return false;
  }
  ...
  // test abs path must be greater than root abs path
  if (strlen(root_abs_path) >= strlen(test_abs_path)) {
    free(root_abs_path);
    free(test_abs_path);
    return false;
  }

  // the root abs path length characters of both should be the
  // same
  int root_abs_path_len = strlen(root_abs_path);
  if (strncmp(root_abs_path, test_abs_path, root_abs_path_len) != 0 ||
      test_abs_path[root_abs_path_len] != '/') {
    free(root_abs_path);
    free(test_abs_path);
    return false;
  }
