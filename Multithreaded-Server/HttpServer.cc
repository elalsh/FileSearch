...
static void HttpServer_ThrFn(ThreadPool::Task* t) {
  ...
  // STEP 1:
  bool done = false;
  HttpConnection ht_con(hst->client_fd);
  HttpRequest ht_req;
  while (!done) {
    if (!ht_con.GetNextRequest(&ht_req)) {
      close(hst->client_fd);
      done = true;
    }
    HttpResponse ht_response = ProcessRequest(ht_req, hst->base_dir,
                              *(hst->indices));
    if (!ht_con.WriteResponse(ht_response)) {
      close(hst->client_fd);
      done = true;
    }
    if (ht_req.GetHeaderValue("connection") == "close") {
      close(hst->client_fd);
      done = true;
    }
  }
...
static HttpResponse ProcessFileRequest(const string& uri,
  ...
  file_name += url_parser.path().substr(string("/static/").size());

  FileReader file_read(base_dir, file_name);
  string contents;
  if (file_read.ReadFile(&contents)) {
    ret.AppendToBody(contents);

    size_t pos = file_name.find_last_of(".");
    string type = file_name.substr(pos + 1);
    // check condition and set the the content-type
...
static HttpResponse ProcessQueryRequest(const string& uri,
  ...
  vector<hw3::QueryProcessor::QueryResult> query_res;

  ret.set_protocol("HTTP/1.1");
  ret.set_message("Found");
  ret.set_content_type("text/html");
  ret.AppendToBody(kThreegleStr);
  ret.set_response_code(200);

  // Get the query strings
  map<string, string> map = url_parser.args();
  string query = map["terms"];
  if (!query.empty()) {
    to_lower(query);
    vector<string> qr_strs;
    split(qr_strs, query, is_any_of(" "), token_compress_on);
    query_res = qp.ProcessQuery(qr_strs);
    ...
    if (query_res.size() == 0) {
      ...
      ret.AppendToBody(EscapeHtml(query).c_str());
      ret.AppendToBody("</b>\n");
      ret.AppendToBody("<p>\n");
    } else {
      ret.AppendToBody("<p><br>\n");
      ret.AppendToBody(to_string(query_res.size()));
      if (query_res.size() == 1) {
        ret.AppendToBody(" result ");  // 1 match
      } else {
        ret.AppendToBody(" results ");  // > 1 match
      }
      ...
      for (size_t i = 0; i < query_res.size(); i++) {
        ret.AppendToBody("<li><a href=\"/static/" + query_res[i].document_name
                         + "\">");
        ret.AppendToBody(EscapeHtml(query_res[i].document_name) + "</a>");
        ret.AppendToBody(" [" + to_string(query_res[i].rank) + "]<br>\n");
      }
      ret.AppendToBody("</ul>\n");
    }
