...
bool DocIDTableReader::LookupDocID(
  ...
  for (IndexFileOffset_t& curr_element : elements) {
    ...
    Verify333(fread(reinterpret_cast<void *>(&curr_header),
              sizeof(DocIDElementHeader), 1, file_) == 1);
    ...
    if (curr_header.doc_id == doc_id) {
      ...
      list<DocPositionOffset_t> elems;
      DocIDElementPosition curr_position;
      IndexFileOffset_t curr_elem_position = curr_element;
      curr_elem_position += sizeof(DocIDElementHeader);
      for (int i = 0; i < curr_header.num_positions; i++) {
        Verify333(fseek(file_, curr_elem_position, SEEK_SET) == 0);
        Verify333(fread(reinterpret_cast<void *>(&curr_position),
                  sizeof(DocIDElementPosition), 1, file_) == 1);
        curr_position.ToHostFormat();
        elems.push_back(curr_position.position);
        curr_elem_position += sizeof(DocIDElementPosition);
      }
...
list<DocIDElementHeader> DocIDTableReader::GetDocIDList() const {
  ...
  for (int i = 0; i < header_.num_buckets; i++) {
    ...
    Verify333(fread(reinterpret_cast<void *>(&bucket_rec), sizeof(BucketRecord),
              1, file_) == 1);
    ...
    for (int j = 0; j < bucket_rec.chain_num_elements; j++) {
      ...
      Verify333(fread(reinterpret_cast<void *>(&element_pos),
      ...
      Verify333(fread(reinterpret_cast<void *>(&element),
                sizeof(DocIDElementHeader), 1, file_) == 1);
