...
static int WriteHeader(FILE* f, int doctable_bytes, int memidx_bytes) {
  ...
  // read the content of the file f, byte by byte and encode using crc
  int length = doctable_bytes + memidx_bytes;
  ...
  for (int i = 0; i < length; i++) {
    data = fgetc(f);
    crc.FoldByteIntoCRC(static_cast<uint8_t>(data));
  }
...
static int WriteHashTable(FILE* f, IndexFileOffset_t offset, HashTable* ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    ...
    int32_t num_elem = static_cast<int32_t>(LinkedList_NumElements(list));
    int bu_size = WriteHTBucketRecord(f, record_pos, num_elem , bucket_pos);
    if (bu_size == kFailedWrite) {
      return kFailedWrite;
    }
    record_pos += bu_size;
    if (num_elem  != 0) {
      int bu_cont = WriteHTBucket(f, bucket_pos, list, fn);
      if (bu_cont == kFailedWrite) {
        return kFailedWrite;
      }
      bucket_pos += bu_cont;
    }
...
static int WriteHTBucket(FILE* f, IndexFileOffset_t offset, LinkedList* li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    int ContByte = fn(f, element_pos, kv);
    if (ContByte == kFailedWrite) {
      return kFailedWrite;
    }

    // Advance to the next element in the chain, updating our offsets.
    record_pos += sizeof(ElementPositionRecord);
    element_pos += ContByte;  // you probably want to change this
    LLIterator_Next(it);
  }
  LLIterator_Free(it);
...
static int WriteDocidToDocnameFn(FILE* f, IndexFileOffset_t offset,
  ...
  if (fwrite((static_cast<char *>(kv->value)), sizeof(char), file_name_bytes, f)
...
static int WriteDocIDToPositionListFn(FILE* f,
  ...
  for (int i = 0; i < num_positions; i++) {
    ...
    IndexFileOffset_t added = sizeof(DocIDElementPosition) * i;
    if (fseek(f, offset + sizeof(DocIDElementHeader) + added, SEEK_SET) != 0) {
      return kFailedWrite;
    }
