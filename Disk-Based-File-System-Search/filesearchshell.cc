...
// Gets the query result for the qury and prints them in appropriate
// order
// Arguments:
// - qp: The query processot object instance
// - query: The query to process
static void GetAndPrintQuery(const QueryProcessor& qp, string query);
...
int main(int argc, char** argv) {
  ...
  QueryProcessor processor(index_list, true);
  while (1) {
    // read user input and pass the query token to  reader and printer function
    ...
    GetAndPrintQuery(processor, queries);
  }
...
static void GetAndPrintQuery(const QueryProcessor& qp, string query) {
  ...
  vector<QueryProcessor::QueryResult> res = qp.ProcessQuery(tokens);

  // print in correct format
  if (res.size() == 0) {
    cout << "  [no results]" << endl;
  }
  for (QueryProcessor::QueryResult& elem : res) {
    cout << "  " << elem.document_name << " (" << elem.rank << ")" << endl;
  }
}
