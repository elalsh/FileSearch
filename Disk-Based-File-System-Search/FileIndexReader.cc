...
FileIndexReader::FileIndexReader(const string& file_name,
  ...
  Verify333(fread(reinterpret_cast<void *>(&header_), sizeof(IndexFileHeader),
        1, file_) == 1);
  ...
  if (validate) {
    ...
    while (left_to_read > 0) {
      ...
      int seek_set = sizeof(IndexFileHeader) + header_.doctable_bytes +
                    header_.index_bytes - left_to_read;
      Verify333(fseek(file_, seek_set, SEEK_SET) == 0);
      int num_read = fread(buf, sizeof(char), kBufSize, file_);
      Verify333(num_read != 0);
      int i = 0;
      while (i < num_read) {
